...
void Dialog1::on_pushButton_valider_clicked()
{
    ...
    querySelect.prepare("SELECT * FROM ANIMAUX WHERE IDAN = :identifiant");
    ...
    if (querySelect.exec() && querySelect.next()) {
        ...
        tableWidget->setItem(0, 0, new QTableWidgetItem(querySelect.value("IDAN").toString()));
        ...
        tableWidget->setItem(0, 5, new QTableWidgetItem(querySelect.value("NOURRITURE").toString()));
