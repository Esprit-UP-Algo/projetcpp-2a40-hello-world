...
bool agri::ajouter() // ajputer dans le tableau
{
    ...
    query.prepare("INSERT INTO agriculture(IDA,TYPE,DATEP,QUANTITE,TDP,PDA,PDV) VALUES(:ida,:type,:datep,:quantite,:tdp,:pda,:pdv)");
...
bool agri:: supprimer(QString ida) // suppression selon l'id
{
    ...
    QSqlQuery query;
    checkQuery.prepare("SELECT IDA FROM agriculture where ida=:ida");
    checkQuery.bindValue(0, ida);
    if (checkQuery.exec()&&checkQuery.next())
    {
        QSqlQuery deleteQuery;
        deleteQuery.prepare("Delete from agriculture where ida=:ida");
...
QSqlQueryModel* agri::afficher() // affichage du tableau
{
    ...
    QSqlQuery query;
        model->setQuery("SELECT * FROM agriculture");
...
QSqlQueryModel* agri::afficher1()

{
    QSqlQueryModel* model = new QSqlQueryModel();
    model->setQuery("SELECT * FROM agriculture ORDER BY quantite");
    model->setHeaderData(0,Qt::Horizontal,QObject::tr("identifiant"));
    model->setHeaderData(1, Qt::Horizontal,QObject::tr("type de plante"));
    model->setHeaderData(2, Qt::Horizontal,QObject::tr("type_de_pesticide"));
    model->setHeaderData(3, Qt::Horizontal,QObject::tr("d_d_plantation"));
...
bool agri::updateData( QString ida) // hedhy il update
{
    ...
    QString res1= QString::number(pdv);
    QString res2= QString::number(quantite);
    query.prepare("UPDATE agriculture SET TYPE=:type,TDP=:tdp,DATEP=:datep,PDA=:pda,PDV=:pdv,QUANTITE=:quantite  WHERE IDA=:ida");
    query.bindValue(":type",type);
    query.bindValue(":tdp", tdp);
    query.bindValue(":datep",datep);
    query.bindValue(":pda",res);
...
{
    QSqlQuery query("SELECT ida, quantite, pdv, pda FROM agriculture");
