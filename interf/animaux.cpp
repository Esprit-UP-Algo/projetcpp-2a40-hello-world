...
animaux::animaux(int identifiant,QString categorie,float masse,float taille,QString nourriture,QString etat) {
...
bool animaux::ajouter() {
    QSqlQuery query;
    query.prepare("INSERT INTO ANIMAUX(IDAN,MASSE,CATEGORIE,TAILLE,nourriture,ETAT) VALUES (:IDENTIFIANT,:MASSE,:CATEGORIE,:TAILLE,:IDA,:ETAT)");
    query.bindValue(":IDENTIFIANT", identifiant);
    query.bindValue(":CATEGORIE", categorie);
    query.bindValue(":MASSE", masse);
    query.bindValue(":TAILLE", taille);
    query.bindValue(":ETAT", etat);
    query.bindValue(":IDA", nourriture);
    return query.exec();
}

bool animaux::supprimer(int identifiant) {
    QSqlQuery query;
    query.prepare("DELETE FROM ANIMAUX WHERE IDAN=:IDENTIFIANT");
    query.bindValue(":IDENTIFIANT", identifiant);
...
QSqlQueryModel* animaux::afficher() {
    ...
    model->setHeaderData(4, Qt::Horizontal, QObject::tr("NOURITTURE"));
    model->setHeaderData(5, Qt::Horizontal, QObject::tr("ETAT"));
    return model;
}

bool animaux::Modifier(int identifiant, QString categorie, float masse, float taille,QString etat,QString nourriture)
{


    QSqlQuery query;
    QString identifiant_string= QString(identifiant);
          query.prepare("UPDATE ANIMAUX SET CATEGORIE=:CATEGORIE,MASSE=:MASSE,TAILLE=:TAILLE,ETAT=:ETAT,nourriture=:IDA WHERE IDAN=:IDENTIFIANT;");
          query.bindValue(":IDENTIFIANT", identifiant);
          query.bindValue(":CATEGORIE", categorie);
          query.bindValue(":MASSE", masse);
          query.bindValue(":TAILLE", taille);
...
QSqlQueryModel* animaux::chercher(QString critere, QString rech) {
    ...
    QString querystr = "SELECT * FROM  ANIMAUX WHERE " + critere + " LIKE :rech";
    QSqlQuery query;
...
{
   QSqlQueryModel * model=new QSqlQueryModel();
   model->setQuery("SELECT * FROM ANIMAUX ORDER BY IDAN");
...
{
   QSqlQueryModel * model=new QSqlQueryModel();
   model->setQuery("SELECT * FROM ANIMAUX ORDER BY TAILLE ");
